...
def _dynamic_dict(example, mt_field, pe_field):
    """Create copy-vocab and numericalize with it.

    In-place adds ``"mt_map"`` to ``example``. That is the copy-vocab
    numericalization of the tokenized ``example["mt"]``. If ``example``
    has a ``"pe"`` key, adds ``"alignment"`` to example. That is the
    copy-vocab numericalization of the tokenized ``example["pe"]``. The
    ...
    Args:
        example (dict): An example dictionary with a ``"src"`` key and
            maybe a ``"pe"`` key. (This argument changes in place!)
        src_field (torchtext.data.Field): Field object.
        mt_field (torchtext.data.Field): Field object.
        pe_field (torchtext.data.Field): Field object.

    Returns:
        torchtext.data.Vocab and ``example``, changed as described.
    """

    mt = mt_field.tokenize(example["mt"])
    # make a small vocab containing just the tokens in the source sequence
    unk = mt_field.unk_token
    pad = mt_field.pad_token
    mt_ex_vocab = Vocab(Counter(mt), specials=[unk, pad])
    unk_idx = mt_ex_vocab.stoi[unk]
    # Map source tokens to indices in the dynamic dict.
    mt_map = torch.LongTensor([mt_ex_vocab.stoi[w] for w in mt])
    example["mt_map"] = mt_map
    example["mt_ex_vocab"] = mt_ex_vocab

    if "pe" in example:
        pe = pe_field.tokenize(example["pe"])
        mask = torch.LongTensor(
            [unk_idx] + [mt_ex_vocab.stoi[w] for w in pe] + [unk_idx])
        example["alignment"] = mask
    return mt_ex_vocab, example


class DatasetForAPE(TorchtextDataset):
    ...
    Args:
        fields (dict[str, Field]): a dict with the structure
            ...
            that means the dataset side, ``"src"``, ``"mt"`` or ``"pe"``.
            Keys match the keys of items yielded by the ``readers``, while values
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        can_copy = "mt_map" in fields and "alignment" in fields

        read_iters = [r.read(dat[1], dat[0], dir_) for r, dat, dir_
                      in zip(readers, data, dirs)]

        # self.mt_vocabs is used in collapse_copy_scores and Translator.py
        self.mt_vocabs = []
        examples = []
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            ...
            if can_copy:
                mt_field = fields["mt"]
                pe_field = fields["pe"]
                # this assumes mt_field and pe_field are both text
                mt_ex_vocab, ex_dict = _dynamic_dict(
                    ex_dict, mt_field.base_field, pe_field.base_field)
                self.mt_vocabs.append(mt_ex_vocab)
